...
;Source/Engine/Components/../Libraries/vec2.h:65: }
	add	sp, #8
	ret
;Source/Engine/axion_engine.c:35: void main(void)
;	---------------------------------
; Function main
; ---------------------------------
_main::
	add	sp, #-3
;Source/Engine/axion_engine.c:39: Entity player =  { .ID = 1, .components = 0 }; // Write function to get next ID
	ldhl	sp,	#0
	ld	e, l
	ld	d, h
	ld	(hl), #0x01
	...
	inc	bc
	ld	(bc), a
;Source/Engine/axion_engine.c:41: EntityAddComponent(&player, TRANSFORM_COMPONENT);
	push	de
	ld	bc, #0x0001
	call	_EntityAddComponent
	pop	de
;Source/Engine/axion_engine.c:42: EntityAddComponent(&player, SPRITE_COMPONENT);
	ld	bc, #0x0002
	call	_EntityAddComponent
;/opt/gbdk/include/gb/gb.h:811: __asm__("di");
	di
;Source/Engine/axion_engine.c:46: DISPLAY_OFF;
	call	_display_off
;Source/Engine/axion_engine.c:47: LCDC_REG = LCDCF_OFF | LCDCF_WIN9C00 | LCDCF_WINON | LCDCF_BG8800 | LCDCF_BG9800 | LCDCF_OBJ16 | LCDCF_OBJON | LCDCF_BGON;
	ld	a, #0x67
	ldh	(_LCDC_REG + 0), a
;Source/Engine/axion_engine.c:62: BGP_REG = OBP0_REG = OBP1_REG = 0xE4U;
	ld	a, #0xe4
	ldh	(_OBP1_REG + 0), a
	ld	a, #0xe4
	ldh	(_OBP0_REG + 0), a
	ld	a, #0xe4
	ldh	(_BGP_REG + 0), a
;Source/Engine/axion_engine.c:65: set_bkg_data(0xFC, 0x04, std_data);
	ld	de, #_std_data
	push	de
	ld	hl, #0x4fc
	push	hl
	call	_set_bkg_data
	add	sp, #4
;Source/Engine/axion_engine.c:66: set_bkg_data(0x00, 0x2D, bkg_data);
	ld	de, #_bkg_data
	push	de
	ld	hl, #0x2d00
	push	hl
	call	_set_bkg_data
	add	sp, #4
;Source/Engine/axion_engine.c:69: set_sprite_data(0x00, 0x1C, earth_data);
	ld	de, #_earth_data
	push	de
	ld	hl, #0x1c00
	push	hl
...
;/opt/gbdk/include/gb/gb.h:1946: shadow_OAM[nb].prop=prop;
	...
	ld	hl, #(_shadow_OAM + 7)
	ld	(hl), #0x00
;Source/Engine/axion_engine.c:74: DISPLAY_ON;
	ldh	a, (_LCDC_REG + 0)
	or	a, #0x80
	ldh	(_LCDC_REG + 0), a
;/opt/gbdk/include/gb/gb.h:795: __asm__("ei");
	ei
;Source/Engine/axion_engine.c:77: while(true) {
00102$:
	jr	00102$
;Source/Engine/axion_engine.c:82: }
	add	sp, #3
	ret
	.area _CODE
	.area _INITIALIZER
